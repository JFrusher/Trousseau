...
const HANDLE_KEY = "brigade.fileSink.handle.v1";
 ...
 */
export async function connect(suggestedName = "crew.brigade.json"): Promise<string | null> {
  if (!isSupported()) return null;
  try {
    handle = await (window as unknown as PickerWindow).showSaveFilePicker({
      suggestedName,
      types: [{ description: "Brigade crew", accept: { "application/json": [".json"] } }],
    });
