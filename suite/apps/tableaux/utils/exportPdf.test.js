...
const roomy = { minX: 0, minY: 0, width: 400, height: 300, basePx: 5 }
const AVAIL_W = 746
const AVAIL_H = 547

describe('planSheets', () => {
  it('keeps one sheet when the type is legible', () => {
    expect(planSheets(roomy, AVAIL_W, AVAIL_H, 'auto').windows).toHaveLength(1)
  })

  it('tiles automatically when one sheet would go below the font floor', () => {
    const { windows, basePt } = planSheets(tiny, AVAIL_W, AVAIL_H, 'auto')
    expect(windows).toHaveLength(2)
    expect(basePt).toBeGreaterThan(planSheets(tiny, AVAIL_W, AVAIL_H, 'single').basePt)
  })

  it('forces a single sheet even when the type is illegible', () => {
    expect(planSheets(tiny, AVAIL_W, AVAIL_H, 'single').windows).toEqual([
      { minX: 0, minY: 0, width: 4000, height: 1000 },
    ])
  })

  it('forces two overlapping sheets even when one would do', () => {
    const { windows } = planSheets(roomy, AVAIL_W, AVAIL_H, 'split')
    expect(windows).toHaveLength(2)
