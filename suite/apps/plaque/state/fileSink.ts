...
const HANDLE_KEY = "plaque.fileSink.handle.v1";
 ...
 */
export async function connect(suggestedName = "cards.plaque.json"): Promise<string | null> {
  if (!isSupported()) return null;
  try {
    handle = await (window as unknown as PickerWindow).showSaveFilePicker({
      suggestedName,
      types: [{ description: "Plaque project", accept: { "application/json": [".json"] } }],
    });
