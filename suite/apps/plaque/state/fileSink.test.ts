...
const fakeHandle = (name = "cards.plaque.json", permission: PermissionState = "granted") => {
...
// A window object is all the sink needs; this repo's tests run in node.
beforeAll(() => {
  (globalThis as unknown as Record<string, unknown>).window ??= {};
});

const givePicker = (handle: unknown) => {
  (globalThis.window as unknown as Record<string, unknown>).showSaveFilePicker = vi.fn(async () => handle);
};

beforeEach(() => store.clear());
afterEach(async () => {
  await disconnect();
  delete (globalThis.window as unknown as Record<string, unknown>).showSaveFilePicker;
});
...
describe("connecting", () => {
  it("remembers the chosen file and writes to it", async () => {
    ...
    expect(await connect()).toBe("cards.plaque.json");
    expect(linkedName()).toBe("cards.plaque.json");
  ...
  it("stays unlinked when the user dismisses the picker", async () => {
    (globalThis.window as unknown as Record<string, unknown>).showSaveFilePicker = vi.fn(async () => {
...
async function disconnect_keepingStorage() {
  const kept = store.get("plaque.fileSink.handle.v1");
  await disconnect();
  store.set("plaque.fileSink.handle.v1", kept);
}
