...
/**
 * Named imports rather than a directory glob.
 *
 * Plaque used Vite's `import.meta.glob`, which has no equivalent here. Listing
 * them is duller and one line longer per template, but it is also the only
 * version a bundler can check: a renamed file becomes a build error instead of
 * a gallery that quietly ships one design short.
 */
