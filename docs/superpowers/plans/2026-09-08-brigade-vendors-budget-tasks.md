...
**Complete — all 6 tasks, 2026-09-08.** 1,647 tests across five projects, 98 in
the contract package, typecheck and build clean, no new dependencies.

**What executing it found.** Making `Job.blockId` nullable was the useful part:
the compiler pointed at three places that had quietly assumed a job always has
a block, and each was wrong in the same way. `orphanJobs` in `crewActions.ts`
and `reconcile.ts`'s orphan report would both have swept every deliberate task
into "work that has lost its place", and `JobPanel` had no way to express "not
tied to the day" at all — so the null could be written but never chosen.

`Board.tsx` needed less than the plan expected and more than it looked: it
*already* rendered blockless jobs, because its orphan filter is "no block
matches" and a null matches nothing. The work was telling the two cases apart,
not adding a list.

The plan also missed `lib/model/crewActions.ts` and
`apps/brigade/core/model/defaults.ts` entirely. Both surfaced as type errors,
which is the argument for changing the type before the UI.
...
- [x] **Step 3: Add the fields to the suite's types**
...
- [x] **Step 4: Fix and extend `readCrew`**
...
- [x] **Step 1: Match the suite's shape**
...
- [x] **Step 2: Seed a new team with them**
...
- [x] **Step 3: Type-check and run Brigade**
...
- [x] **Step 1: Add the fields to the team editor**
...
- [x] **Step 1: Separate the two kinds**
...
- [x] **Step 2: Let a job be detached**
...
- [x] **Step 3: Confirm the validator still agrees**
...
- [x] **Step 5: A wedding with none of this still looks untouched**
